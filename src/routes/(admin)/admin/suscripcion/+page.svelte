<script lang="ts">
    ...
    $: diasVencidos =
        suscripcion && suscripcion.diasRestantes < 0
            ? Math.abs(suscripcion.diasRestantes)
            : 0;

    // Asumimos un trial estándar de 15 días para la barra de progreso
    $: porcentajeTrial = estaTrial
        ? Math.min(100, Math.max(0, (diasRestantes / 15) * 100))
        : 0;
...
{:else}
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div class="lg:col-span-7 space-y-6">
            {#if estaVencida}
                <div
                    class="bg-danger-50 border-2 border-danger-200 rounded-2xl p-6 shadow-sm relative overflow-hidden"
                >
                    <div class="relative z-10 flex items-start gap-4">
                        <div
                            class="flex-shrink-0 w-12 h-12 bg-danger-100 rounded-full flex items-center justify-center"
                        >
                            <svg
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-danger-800 mt-1">
                                El acceso a las funciones completas está
                                bloqueado. Por favor, renueva tu plan para
                                continuar utilizando el sistema sin
                                interrupciones.
                            </p>
                            ...
                            <div class="mt-4">
                                <button
                                    onclick={irARenovar}
                                    class="px-5 py-2.5 bg-danger-600 hover:bg-danger-700 text-white font-medium rounded-lg shadow-sm transition-colors shadow-danger-200 animate-[pulse_2s_ease-in-out_infinite]"
                                >
                                    Renovar plan ahora
                                </button>
                            </div>
            ...
            >
                <div
                    class="p-5 border-b border-gray-100 flex flex-wrap items-center justify-between gap-4"
                >
                    <div>
                        <p
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <Badge variant={badgeVariant} dot
                                >{suscripcion.estado.toUpperCase()}</Badge
                            >
                        </div>
                    </div>
                    {#if !estaVencida}
                        <Button variant="secondary" onclick={irARenovar}
                            >Cambiar o Renovar</Button
                        >
                    {/if}
                </div>
                ...
                {#if estaTrial && !estaVencida}
                    <div class="p-5 bg-primary-50 border-b border-primary-100">
                        <div class="flex justify-between items-end mb-2">
                            ...
                            {#if esBienvenida}
                                <span
                                    class="text-xs font-bold text-primary-600 uppercase"
                                    >¡Bienvenido!</span
                                >
                            {/if}
                        </div>
                        <div
                            class="w-full bg-primary-200 rounded-full h-2.5 overflow-hidden mt-3"
                        >
                            <div
                ...
                <div
                    class="grid grid-cols-2 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-100"
                >
